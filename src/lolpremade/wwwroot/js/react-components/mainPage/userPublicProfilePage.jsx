...
class OpinionComponent extends React.Component {
    ...
    getUserName() {
        ...
        return apiService.GetById('users',this.props.opinion.senderUserId).then(function(user) {
            return user;
        }.bind(this))
    }
    componentDidMount() {
        this.getUserName().then(function(user) {
            this.setState({
    ...
    render() {
        return <article>
                    <header>Comentario de: { this.state.username }</header>
...
class UserPublicProfileComponent extends React.Component {
    ...
    render() {
        return <div className="center-block text-center">
                    <img height="200" width="400" src={this.state.user.userAvatar }></img>
